...
}, handleEmailContent = function () {
    $("#wysihtml5").wysihtml5()
}, handleAddCc = function () {
    $(document).on("click", '[data-click="add-cc"]', function (a) {
        a.preventDefault();
        var t = $(this).attr("data-name"), l = "email-cc-" + t,
            n = '\t<div class="email-to">\t\t<label class="control-label">' + t + ':</label>\t\t<ul id="' + l + '" class="primary line-mode"></ul>\t</div>';
        $('[data-id="extra-cc"]').append(n), $("#" + l).tagit(), $(this).remove()
    })
}, EmailCompose = function () {
    "use strict";
    return {
        init: function () {
            handleEmailToInput(), handleEmailContent(), handleAddCc()
        }
    }
}();